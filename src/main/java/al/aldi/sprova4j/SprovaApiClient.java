...
public class SprovaApiClient {
    ...
    public TestCase filterTestCaseByTestSetId(String testSetId, String jsonFiler) throws TestCaseException {
        TestCase result = null;

        try {
            result = TestCase.toObject(post(String.format("%s/%s/%s/findOne", API_TESTSETS, testSetId, TESTCASES), jsonFiler));
            if (result == null) {
                throw new TestCaseException("Test case not found => filter: " + jsonFiler);
            }
            result.setClient(this);
    ...
    public List<TestSetExecution> getTestSetExecutionsByTestSetId(String cycleId) {
        ...
        try {
            result = TestSetExecution.toObjects(get(String.format("%s/%s/%s", API_TESTSETS, cycleId, TESTSETS_EXECUTIONS)));
